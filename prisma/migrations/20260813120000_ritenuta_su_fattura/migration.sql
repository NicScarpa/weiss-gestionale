...
-- ritenuta è sospeso: è un canale di saldo con ciclo di vita proprio (F24),
-- non una riga da girare a conto. Leggerla e non salvarla la perderebbe; ecco
-- perché la colonna esiste prima che esista una schermata che la usa.
--
-- Nullable e senza backfill: le fatture già importate restano `null`, che è
-- il loro stato vero — l'unica fonte che può popolare il campo è il flusso di
...
-- mai interrogato da solo. Stessa scelta già fatta per `line_items`,
