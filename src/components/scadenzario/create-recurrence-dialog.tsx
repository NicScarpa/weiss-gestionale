...
export function CreateRecurrenceDialog({
  ...
  // Options from API: restano qui perché il dialog le chiede una volta per
  // apertura. Nel modulo interno verrebbero richieste di nuovo a ogni
  // rimontaggio, chiusura compresa.
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto bg-white">
        ...
        {/* Radix smonta il contenuto alla chiusura e la `key` lo rifà da capo
            quando si passa da una ricorrenza all'altra senza chiudere: i campi
...
function ModuloRicorrenza({
  ...
  const [importo, setImporto] = useState(initialData ? String(Number(initialData.importo)) : '')
  const [categoriaId, setCategoriaId] = useState<string>(initialData?.categoriaId ?? '')
  const [contoDiPagamentoId, setContoDiPagamentoId] = useState<string>(initialData?.contoDiPagamentoId ?? '')
  const [metodoPagamento, setMetodoPagamento] = useState<string>(initialData?.metodoPagamento ?? '')
  const [frequenza, setFrequenza] = useState<string>(initialData?.frequenza ?? RecurrenceType.MENSILE)
  const [giornoDelMese, setGiornoDelMese] = useState<string>(initialData?.giornoDelMese?.toString() ?? '1')
  const [giornoDellSettimana, setGiornoDellSettimana] = useState<string>(initialData?.giornoDellSettimana?.toString() ?? '0')
  const [dataInizio, setDataInizio] = useState<Date>(
    () => initialData ? new Date(initialData.dataInizio) : new Date()
  )
  const [hasDataFine, setHasDataFine] = useState(!!initialData?.dataFine)
  const [dataFine, setDataFine] = useState<Date | undefined>(
    () => initialData?.dataFine ? new Date(initialData.dataFine) : undefined
  )
