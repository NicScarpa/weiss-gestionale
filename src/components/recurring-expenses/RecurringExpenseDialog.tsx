    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        ...
        {/* Radix smonta il contenuto alla chiusura e la `key` lo rifà da capo
            quando si passa da una spesa all'altra senza chiudere: i campi
...
function ModuloSpesa({
  ...
  const [name, setName] = useState(spesa?.name ?? '')
  const [description, setDescription] = useState(spesa?.description ?? '')
  const [amount, setAmount] = useState(spesa ? String(Number(spesa.amount)) : '')
  const [frequency, setFrequency] = useState<Frequenza>(spesa?.frequency ?? 'MONTHLY')
  const [dayOfMonth, setDayOfMonth] = useState(String(spesa?.dayOfMonth ?? 1))
  const [dayOfWeek, setDayOfWeek] = useState(String(spesa?.dayOfWeek ?? 1))
  const [startDate, setStartDate] = useState(() => giornoCivile(spesa?.startDate))
  const [endDate, setEndDate] = useState(() => giornoCivile(spesa?.endDate))
  const [category, setCategory] = useState(spesa?.category ?? '')
