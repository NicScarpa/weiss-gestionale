...
describe('calculateScheduleMatchScore', () => {
  it('dà punteggio pieno a importo e data coincidenti', () => {
    const score = calculateScheduleMatchScore(movimento(), scadenza())
  ...
  it('non propone un movimento nel verso sbagliato', () => {
    ...
    expect(calculateScheduleMatchScore(incasso, scadenza())).toBe(0)
  })
  ...
  it('salda una scadenza attiva con un incasso, non con un\'uscita', () => {
    ...
    expect(calculateScheduleMatchScore(incasso, attiva)).toBeGreaterThan(0.8)
    expect(calculateScheduleMatchScore(uscita, attiva)).toBe(0)
  })

  it('ignora le scadenze già saldate', () => {
    const saldata = scadenza({ importoPagato: 1000 })
    expect(calculateScheduleMatchScore(movimento(), saldata)).toBe(0)
  })
  ...
  it('confronta il movimento col residuo, non col totale', () => {
    ...
    expect(calculateScheduleMatchScore(saldo, parziale)).toBeGreaterThanOrEqual(0.9)
  })
  ...
  it('considera plausibile un acconto, ma meno di un saldo esatto', () => {
    ...
    const scoreAcconto = calculateScheduleMatchScore(acconto, scadenza())
    const scoreSaldo = calculateScheduleMatchScore(saldo, scadenza())
  ...
  it('riconosce il numero documento nella causale', () => {
    ...
    expect(calculateScheduleMatchScore(conRiferimento, scadenza())).toBeGreaterThan(
      calculateScheduleMatchScore(senzaRiferimento, scadenza())
    )
  })
  ...
  it('propone il match anche quando la controparte non coincide', () => {
    ...
    expect(calculateScheduleMatchScore(altroNome, bolletta)).toBeGreaterThanOrEqual(
  ...
  it('non supera mai il punteggio massimo', () => {
    ...
    expect(calculateScheduleMatchScore(perfetto, scadenza())).toBeLessThanOrEqual(1)
  })
})
