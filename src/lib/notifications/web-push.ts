...
function preparaCanale(): boolean {
  ...
  if (!config) {
    // Una riga sola per processo: senza, un deploy a cui manca una delle tre
    // variabili VAPID smette di notificare in perfetto silenzio.
    if (!avvisato) {
      logger.warn('Chiavi VAPID non configurate: nessuna notifica push verrà inviata')
      avvisato = true
    }
    return false
  }
