...
- **Ogni rotta avvolge il proprio corpo in `try/catch` e risponde con `rispostaErroreGoCardless(errore, 'METODO /api/...')`.** `withAuth` non intercetta le eccezioni dell'handler, e le altre ventitré rotte del progetto avvolgono tutte: una rotta che non lo fa trasforma qualunque imprevisto in un 500 anonimo. Qui, in più, ci sono due segnali che **non devono** diventare un 500: il contingente esaurito, che è l'informazione più preziosa dell'integrazione e va detta con i secondi che mancano, e le chiavi non configurate, che è un problema di installazione e non un guasto. La funzione sta in `src/lib/gocardless/risposte.ts` e mappa `LimiteRaggiunto` → 429, `ConfigurazioneMancante` → 503, `ErroreGoCardless` → 502, e delega tutto il resto a `handleApiError`. Introdotta nel Task 3; i task successivi la usano e basta.
